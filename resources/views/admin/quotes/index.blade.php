  ...
  <div class="lg:hidden sticky top-0 z-20 glass-panel border-b border-white/10 px-4 py-4 mb-4">
    <div class="flex items-center justify-between gap-3">
      ...
      <div class="text-xs text-gray-400 shrink-0">
        Total: <span class="text-white font-semibold">{{ $quotes->total() }}</span>
      </div>
    </div>
  </div>
        ...
        <table class="min-w-full text-sm">
          ...
          <tbody class="divide-y divide-white/10">
            ...
            <tr class="hover:bg-white/[0.03]">
              <td class="px-4 py-3 text-white whitespace-nowrap">{{ $q->full_name }}</td>
              <td class="px-4 py-3 text-gray-300 whitespace-nowrap">{{ $q->email }}</td>
              <td class="px-4 py-3 text-gray-400 whitespace-nowrap">{{ $q->phone }}</td>
              <td class="px-4 py-3 text-gray-400 whitespace-nowrap">{{ $q->area }}</td>
              <td class="px-4 py-3 text-gray-400 whitespace-nowrap">{{ $q->quote_type }}</td>
              <td class="px-4 py-3 text-gray-400 whitespace-nowrap">
                @if(!is_null($q->budget))
                {{ number_format((float)$q->budget, 2) }}
                @else
                —
                @endif
              </td>
              ...
              <td class="px-4 py-3 text-gray-400 whitespace-nowrap">
                {{ $q->att_count ? $q->att_count.' file(s)' : '—' }}
              </td>

              <td class="px-4 py-3 whitespace-nowrap">
                <span class="px-2 py-1 rounded-full text-xs font-bold
                    {{ $q->status === 'new' ? 'bg-yellow-500/20 text-yellow-300 border border-yellow-500/30' : 'bg-green-500/20 text-green-300 border border-green-500/30' }}">
                  {{ strtoupper($q->status) }}
                </span>
              </td>

...
@section('title', 'About Us | Digitron Computers UAE')
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-8">
            ...
            <div class="text-center group">
                <div class="w-16 h-16 rounded-2xl bg-brand-accent/10 flex items-center justify-center mx-auto mb-4 group-hover:bg-brand-accent/20 transition-colors group-hover:scale-110 transform duration-300">
                    <i class="bi bi-{{ $stat['icon'] }} text-3xl text-brand-accent"></i>
                </div>
                <div class="text-3xl md:text-4xl font-display font-bold text-white mb-1 counter-up" data-target="{{ $stat['number'] }}">{{ $stat['number'] }}</div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="relative">
            ...
            <div class="space-y-24">
                ...
                <div class="relative flex items-center {{ $milestone['side'] === 'right' ? 'md:flex-row-reverse' : '' }} group">
                    <!-- Content -->
                    <div class="w-full md:w-1/2 {{ $milestone['side'] === 'right' ? 'md:pl-16' : 'md:pr-16' }}">
                        <div class="glass-panel rounded-2xl p-6 border border-white/10 hover:border-brand-accent/30 transition-all duration-500 hover:-translate-y-2 {{ $milestone['side'] === 'right' ? 'md:text-left' : 'md:text-right' }}">
                            <div class="aspect-video rounded-xl overflow-hidden mb-6 relative">
                                ...
                                <div class="absolute bottom-4 {{ $milestone['side'] === 'right' ? 'left-4' : 'right-4' }}">
                                    <span class="text-5xl font-display font-bold text-white/20">{{ $milestone['year'] }}</span>
                                </div>
                            </div>
                            ...
                            <p class="text-gray-400 leading-relaxed">{{ $milestone['desc'] }}</p>
                        </div>
                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($features as $index => $feature)
                <div class="group relative rounded-2xl p-8 border border-white/12 bg-white/[0.06] backdrop-blur-xl
                            {{ $feature['ring'] }} transition-all duration-500 hover:-translate-y-3 overflow-hidden">
                    <div class="absolute inset-0 bg-gradient-to-br {{ $feature['glow'] }} to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>

                    <div class="relative w-16 h-16 rounded-2xl {{ $feature['iconBg'] }} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300">
                        <i class="bi bi-{{ $feature['icon'] }} text-3xl {{ $feature['iconText'] }}"></i>
                    </div>

                    <h3 class="relative text-xl font-bold text-white mb-3 {{ $feature['titleHover'] }} transition-colors">
                        {{ $feature['title'] }}
                    </h3>
                    <p class="relative text-gray-300/80 leading-relaxed">{{ $feature['desc'] }}</p>

                    <div class="absolute top-4 right-4 text-6xl font-display font-bold text-white/5 {{ $feature['numHover'] }} transition-colors">
                        0{{ $index + 1 }}
                    </div>
                </div>
            @endforeach
            ...
            <div class="team-card group relative">
                <div class="glass-panel rounded-2xl overflow-hidden border border-white/10 hover:border-brand-accent/50 transition-all duration-500">
                    ...
                    <div class="p-6 text-center">
                        ...
                        <div class="text-brand-accent text-sm font-medium mb-3">{{ $member['role'] }}</div>
                        <p class="text-sm text-gray-400 leading-relaxed">{{ $member['bio'] }}</p>
                    </div>
                </div>
            </div>
            @endforeach
        ...
        <div class="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-4">
                    ...
                    <a href="{{ route('quote') }}" class="px-6 py-3 rounded-xl border border-white/20 text-white font-bold hover:border-brand-accent hover:text-brand-accent transition-colors flex items-center gap-2">
...
<section class="py-24 relative overflow-hidden">
    ...
    <div class="relative z-10 max-w-4xl mx-auto px-4 text-center">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href="{{ route('quote') }}" class="group px-8 py-4 rounded-xl bg-brand-accent text-black font-bold text-lg hover:bg-white transition-all flex items-center gap-2 shadow-lg shadow-brand-accent/30">
                Start Building <i class="bi bi-cpu group-hover:rotate-12 transition-transform"></i>
            </a>
            <a href="{{ route('shop') }}" class="px-8 py-4 rounded-xl border border-white/20 text-white font-bold text-lg hover:border-brand-accent hover:text-brand-accent transition-all flex items-center gap-2">

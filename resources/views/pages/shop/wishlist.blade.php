...
@section('title', 'My Wishlist | Digitron Computers UAE')
...
<section class="relative h-[45vh] min-h-[460px] overflow-hidden flex items-center justify-center"
    ...
    <div class="absolute inset-0 w-full h-full z-0">
        <img src="{{ asset('images/pexels-ron-lach-7858767.jpg') }}"
            alt="Tech Background"
    ...
    <div class="relative z-40 text-center px-4 pt-24 sm:pt-28 md:pt-32">
        ...
        <div class="flex items-center justify-center gap-10 mt-12 mb-12">
            <div class="text-center">
                <div class="text-2xl font-bold text-white" id="wishlist-count">{{ $wishlistCount }}</div>
                <div class="text-xs text-gray-500 uppercase tracking-wider">Items</div>
            </div>
            <div class="w-px h-10 bg-white/20"></div>
            <div class="text-center">
                <div class="text-2xl font-bold text-brand-secondary" id="wishlist-value">AED {{ number_format($wishlistTotal) }}</div>
...
<section class="py-12 -mt-6 md:-mt-8 relative z-30">
            ...
            <div class="wishlist-item group relative glass-panel rounded-2xl border border-white/10 overflow-hidden hover:border-brand-secondary/50 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-brand-secondary/10"
                ...
                <div class="relative aspect-[4/3] overflow-hidden">
                    ...
                    <div class="absolute top-4 left-4">
                        ...
                        {{ $badge === 'Price Drop' ? 'bg-emerald-500/80 text-white' : '' }}
                        {{ $badge === 'Hot' ? 'bg-orange-500/80 text-white' : '' }}
                        {{ $badge === 'Sale' ? 'bg-brand-accent text-black' : '' }}
                        {{ $badge === 'Restocking Soon' ? 'bg-gray-500/80 text-white' : '' }}
                    ...
                    <div class="absolute top-4 right-4">
                        ...
                        {{ $stockLabel === 'In Stock' ? 'bg-emerald-500/20 text-emerald-400' : '' }}
                        {{ $stockLabel === 'Low Stock' ? 'bg-orange-500/20 text-orange-400' : '' }}
                        {{ $stockLabel === 'Out of Stock' ? 'bg-red-500/20 text-red-400' : '' }}">
                            <span class="w-1.5 h-1.5 rounded-full inline-block mr-1
                            {{ $stockLabel === 'In Stock' ? 'bg-emerald-400 animate-pulse' : ($stockLabel === 'Low Stock' ? 'bg-orange-400' : 'bg-red-400') }}"></span>
                            {{ $stockLabel }}
                        </span>
                    </div>
                    ...
                    transition-all duration-500">
                        <button type="button"
                            class="js-add-to-cart flex-1 py-3 rounded-xl bg-brand-secondary text-white font-bold text-sm hover:bg-brand-accent hover:text-black transition-colors flex items-center justify-center gap-2
                                    {{ $stockLabel === 'Out of Stock' ? 'opacity-50 cursor-not-allowed pointer-events-none' : '' }}"
                            data-id="{{ $product->id }}"
                ...
                <div class="p-5">
                    ...
                    <div class="flex items-center gap-2 mb-2">
                        <div class="flex items-center gap-0.5">
                            @for($i = 0; $i < 5; $i++)
                                <i class="bi bi-star-fill text-xs {{ $i < floor($rating) ? 'text-yellow-400' : 'text-gray-600' }}"></i>
                                @endfor
                        </div>
                    ...
                    <p class="text-sm text-gray-400 mb-4">{{ $product->short_specs ?? $product->specs ?? '' }}</p>

                    <!-- Price -->
                    <div class="flex items-end justify-between">
                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                <span class="text-2xl font-bold text-white">AED {{ number_format($price) }}</span>
                                @if($sale)
                                <span class="text-sm text-gray-500 line-through">AED {{ number_format($original) }}</span>
                                @endif
                            </div>
                            @if($sale)
                            <span class="text-xs text-emerald-400 font-medium">
                                Save AED {{ number_format($original - $price) }}
                            </span>
                            @endif
                        </div>
    ...
    <div id="empty-wishlist" class="hidden glass-panel rounded-2xl p-16 text-center border border-white/10">
        ...
        <a href="{{ route('shop') }}" class="inline-flex items-center gap-2 px-8 py-4 rounded-xl bg-gradient-to-r from-brand-secondary to-brand-accent text-white font-bold hover:shadow-lg hover:shadow-brand-secondary/30 transition-all">
    ...
    <div class="mt-16 pt-16 border-t border-white/10">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <a href="{{ route('product.show', $rec->slug) }}" class="group rounded-xl border border-white/5 bg-white/[0.02] p-4 hover:border-brand-accent/30 transition-all hover:-translate-y-1">
                ...
                <h4 class="text-sm font-medium text-white group-hover:text-brand-accent transition-colors truncate">{{ $rec->name }}</h4>
                <div class="text-sm font-bold text-brand-accent mt-1">AED {{ number_format((int)$rec->price) }}</div>
            </a>
            @endforeach
        </div>
    </div>
